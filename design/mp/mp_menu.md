...
## Application patterns (Паттерны приложения)
 ...
 ### Testing (Тестирование)
   - #### [Service integration contract test (Тестирование контрактов, ориентированное на потребителя)](#service-integration-contract-test)

 ### UI (Графический интерфейс)

 ### Observability (Наблюдаемость)
   - #### [Audit Logging pattern](#audit-logging)
...
#### Проблема
Как понять поведение пользователей и приложения и устранить проблемы? Полезно знать, какие действия недавно выполнял пользователь: 
поддержка клиентов, соблюдение требований, безопасность и т.д.


#### Решение
Запись активности пользователей в базу данных.
...
Этот паттерн имеет следующие недостатки:
- Код аудита переплетается с бизнес-логикой, что усложняет бизнес-логику.


#### Смежные паттерны
- Паттерн [Event Sourcing](#event-sourcing) — надежный способ реализации аудита.
...
#### Контекст
Приложение состоит из множества микросервисов. Микросервисы часто вызывают другие Микросервисы. Вы должны написать автоматические 
тесты, которые проверят правильность работы микросервиса.


#### Проблема
Как легко проверить, что сервис предоставляет API, который ожидают его клиенты? Сквозное тестирование (End to end testing)
(т. е. тесты, запускающие несколько сервисов) является сложным, медленным, нестабильным и дорогостоящим процессом.

#### Решение
Набор тестов для микросервиса, написанный разработчиками другого микросервиса, который его использует. Набор тестов 
проверяет, соответствует ли микросервис ожиданиям микросервиса-потребителя.
...
Этот паттерн имеет следующие недостатки:
- Тесты могут пройти успешно, но приложение не сможет работать в рабочей среде.
- Как гарантировать, что предоставленные потребителем тесты соответствуют тому, что на самом деле требуется потребителю?
