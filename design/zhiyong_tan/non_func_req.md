...
+ [Scalability (Масштабируемость)](#Какие-логические-операции-и-операторы-вы-знаете) - Способность системы легко и без 
особых усилий регулировать использование ресурсов (hardware resource) для экономичной поддержки их нагрузки
+ [Availability (Доступность)](#Что-такое-тернарный-оператор-выбора) - Процент времени, в течение которого система может принимать запросы и возвращать 
желаемый ответ
+ [Performance/latency/P99 and throughput (Производительность/задержка/P99 и пропускная способность)](#Какие-побитовые-операции-вы-знаете) - 
...
+ [Fault-tolerance (Отказоустойчивость)](#Опишите-работу-блока-try-catch-finally) - Способность системы продолжать работу в случае выхода из строя 
...
+ [Final vs Effectively Final](#final-vs-effectively-final)
...
+ Локальные переменные не могут быть изменены после присвоения им значения (+ операции над ними производятся быстрее, например, конкатенация финализированных строк быстрее в 2,5 раза).
...
Объекты или примитивные значения фактически Effectively Final, если мы не изменяем их значения после инициализации. 
В случае с объектами, если мы не изменяем ссылку на объект, она фактически является Effectively Final — даже если изменение 
происходит в состоянии объекта, на который ссылаются.

Самый простой способ понять, действительно ли переменная Final является окончательной, — это подумать, позволит ли удаление ключевого слова Final код скомпилировать и запустить:
...
Переназначение значения или изменение приведенной выше Effectively Final переменной сделает код недействительным независимо 
от того, где он встречается.
...
неявно объявляются как `public abstract` или (начиная с Java 8) `default` - методами с реализацией по-умолчанию, 
...
+ Если проблему необходимо решить с использованием множественного наследования и она состоит из разных иерархий классов;
+ Когда несвязанные классы реализуют наш интерфейс. Например, Comparable предоставляет метод CompareTo(), 
который можно переопределить для сравнения двух объектов;
+ Когда функциональные возможности приложения должны быть определены как контракт, но не важно, кто реализует поведение, 
т. е. сторонним поставщикам необходимо реализовать его полностью.

Когда использовать абстрактный класс:
+ При попытке использовать концепцию наследования в коде (совместное использование кода среди многих связанных классов), 
предоставляя общие методы базового класса, которые переопределяют подклассы;
+ Если у нас есть конкретные требования и только частичные детали реализации;
+ Классы, расширяющие абстрактные классы, имеют несколько общих полей или методов (которые требуют закрытых модификаторов);
+ Если кто-то хочет иметь нефинальные или нестатические методы для изменения состояний объекта.

Абстрактные классы содержат частичную реализацию, которая дополняется или расширяется в подклассах. При этом все 
подклассы схожи между собой в части реализации, унаследованной от абстрактного класса, и отличаются лишь в 
части собственной реализации абстрактных методов родителя. Поэтому абстрактные классы применяются в случае построения 
иерархии однотипных, очень похожих друг на друга классов. В этом случае наследование от абстрактного класса, 
реализующего поведение объекта по умолчанию может быть полезно, так как позволяет избежать написания 
...
## Разница между Strong, Weak, Soft, and Phantom ссылками
+ **Strong** - Объект не может быть доступен для очистки сборщику мусора, если он доступен по любой сильной ссылке. 

Допустим, мы создаем объект ArrayList и присваиваем его переменной списка:
...
Сборщик мусора не может собрать этот список, поскольку мы держим на него строгую ссылку в переменной списка. 
Но если мы затем обнулим переменную, Теперь и только сейчас объект ArrayList можно собрать, поскольку ничто не содержит ссылок на него.
...
+ **Soft** - Все soft ссылки на объекты, доступные только по soft ссылкам, должны быть удалены до того, 
как будет выдано исключение OutOfMemoryError. Объект может оставаться в памяти некоторое время, пока коллектор не решит, 
что ему нужно его собрать. Это произойдет, особенно когда JVM рискует исчерпать память.
...
Мягкие ссылки можно использовать, чтобы сделать наш код более устойчивым к ошибкам, связанным с нехваткой памяти. 
Например, мы могли бы создать кэш, чувствительный к памяти, который автоматически удаляет объекты при нехватке памяти. 
Нам не нужно будет управлять памятью вручную, поскольку за нас это сделает сборщик мусора.

+ **Weak** - Объекты, на которые ссылаются только слабые ссылки, не предотвращаются от сбора. 
С точки зрения сбора мусора они вообще не могли существовать. Если объект со слабой ссылкой должен быть защищен от очистки, 
на него также должна ссылаться какая-либо strong ссылка.

Слабые ссылки чаще всего используются для создания canonicalizing mappings. Это отображения, отображающие только те объекты, 
до которых можно добраться. Отличным примером является WeakHashMap, который работает как обычный HashMap, но на его 
ключи имеются слабые ссылки, и они автоматически удаляются при очистке референта.

Используя WeakHashMap, мы можем создать кратковременный кеш, который очищает объекты, которые больше не 
используются другими частями кода. Если бы мы использовали обычный HashMap, само существование ключа в карте не 
позволило бы его очистить сборщику мусора.

+ **Phantom** - Подобно слабым ссылкам, фантомные ссылки не запрещают сборщику мусора ставить объекты в очередь для очистки. 
Разница в том, что фантомные ссылки необходимо вручную опросить из очереди ссылок, прежде чем их можно будет завершить. 
Это означает, что мы можем решить, что хотим делать, прежде чем они будут очищены.

Фантомные ссылки хороши, если нам нужно реализовать некоторую логику финализации, и они значительно более надежны и 
гибки, чем метод Finalize. Давайте напишем простой метод, который будет проходить через очередь ссылок и выполнять 
очистку всех ссылок:
...
Фантомные ссылки не позволяют нам получить их референт с помощью метода get. По этой причине общепринятой практикой 
является расширение класса PhantomReference собственным, содержащим информацию, важную для логики очистки.

Другими важными вариантами использования фантомных ссылок являются отладка и обнаружение утечек памяти. 
Даже если нам не нужно выполнять какие-либо операции финализации, мы можем использовать фантомные ссылки, чтобы наблюдать, 
какие объекты и когда освобождаются.
...
Это вложенный класс, объявленный с использованием ключевого слова `static`. К классам верхнего уровня 
модификатор `static` неприменим.
...
+ Обычные вложенные классы не могут содержать статических методов, блоков инициализации и классов. 
Статические вложенные классы - могут.
+ В объекте обычного вложенного класса хранится ссылка на объект внешнего класса. 
Внутри статической такой ссылки нет. Доступ к экземпляру обрамляющего класса осуществляется через указание 
