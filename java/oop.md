...
+ [Что такое _статическое(раннее)_ и _динамическое(позднее) связывание_?](#Что-такое-статическоераннее-и-динамическоепозднее-связывание)
+ [Какие еще парадигмы программирования существуют помимо ООП?](#Какие-еще-парадигмы-программирования-существуют-помимо-ООП)
+ [Что подразумевается под структурным программированием?](#Что-подразумевается-под-структурным-программированием)
...
## Что такое _статическое(раннее)_ и _динамическое(позднее) связывание_?
Оба понятия относятся к полиморфизму. Присоединение вызова метода к телу метода называется связыванием.
__Полиморфизм времени компиляции (Compile Time Polymorphism)__. Статический полиморфизм, относится к типу полиморфизма, который происходит во время компиляции. Это означает, что компилятор решает, какую форму или значение должна принять сущность.
Пример:
```java
// В этой программе создаются несколько методов с одним и тем же названием, 
// но компилятор легко решает, какой метод вызывать прямо во время самой компиляции..
...
class Test{
   ...
   // В приведенном ниже примере компилятор просматривает типы аргументов и решает вызвать метод 1.
   System.out.println(demo.add(2,3));
   // Аналогично, в следующей строке кода компилятор вызывает метод 2
   System.out.println(demo.add(2,3,4));
   // Аналогично, в следующей строке кода компилятор вызывает метод 4
   System.out.println(demo.add(2,3.4));
   // Аналогично, в следующей строке кода компилятор вызывает метод 3
   System.out.println(demo.add(2.5,3)); 
   }
}
```
В приведенном выше примере существует четыре версии методов добавления. Первый метод принимает два параметра, а второй — три. Для третьего и четвертого методов происходит изменение порядка параметров. Компилятор просматривает сигнатуру метода и решает, какой метод вызвать для конкретного вызова метода во время компиляции. Создание метода с названием, совпадающим по названию с другим методом, но имеющим разные параметры, называется __Перегрузкой метода (Overload)__

__Полиморфизм времени выполнения (Runtime Polymorphism)__. Динамический полиморфизм, относится к типу полиморфизма, который происходит во время выполнения программы. Компилятор не участвует в процессе принятия решения. Следовательно, какую форму или значение следует принять, зависит от исполнения.
Пример:
```java
class AnyVehicle{
   public void move(){
...
class Test{
   ...
   // В строке выше объект vehicle имеет тип AnyVehicle
   vehicle.move();
   // Но если вызвать переопреденный метод .move(), то получим: «Bike can move too!!», 
   // поскольку фактическая реализация объекта vehicle определяется во время выполнения
   vehicle = new AnyVehicle();
   // Теперь, результат метода .move() будет: “Any vehicle should move!!”, 
   vehicle.move();
   }
}
```
В случае с наследованием мы можем переопределить (override) метод класса-родителя, это по сути и есть пример динамического связывания.
...
Парадигмы программирования — это метод классификации языков программирования на основе их особенностей. В основном существует два типа парадигм программирования:

+ Императивная (Imperative) парадигма программирования
+ Парадигма декларативного (Declarative) программирования

__Императивное программирование__ фокусируется на том, __КАК__ выполнять логику программы, и определяет поток управления как инструкции, которые изменяют состояние программы.  
Это можно дополнительно классифицировать как:
+ _Процедурное программирование_. Определяет шаги, которые программа должна предпринять для достижения желаемого состояния, обычно читаемые в порядке сверху вниз. (Языки: C, Pascal, COBOL, ALGOL, BASIC, Fortran)
+ _Объектно-ориентированное программирование_. Организует программы как объекты, которые содержат некоторые данные (переменные - состояние) и имеют определенное поведение (методы). (Языки: Java, C++, C#, Python, PHP, JavaScript, Ruby, Perl)
+ _Параллельное программирование_. Парадигма параллельного программирования разбивает задачу на подзадачи и фокусируется на их одновременном выполнении. (Языки: Java, Go, C#, Rust)

__Декларативное программирование__ фокусируется на том, __ЧТО__ выполнять, и определяет логику программы, но не подробный поток управления. Декларативная парадигма может быть подразделена на:

+ _Логическое программирование_. Парадигма логического программирования основана на формальной логике, которая относится к набору предложений, выражающих факты и правила о том, как решить проблему. (Языки: Parlog, Prolog)
+ _Функциональное программирование_. Функциональное программирование — это парадигма программирования, в которой программы создаются путем применения и составления функций. (Языки: Haskell, Clojure, Erlang, Elixir)
+ _Программирование базы данных_. модель программирования базы данных используется для управления данными и информацией, структурированной в виде полей, записей и файлов. (SQL)


## Что подразумевается под структурным программированием?

Структурное программирование относится к методу программирования, который состоит из полностью структурированного потока управления (structured control flow). Здесь структура относится к блоку, который содержит набор правил и имеет определенный поток управления, например (if/then/else), (while и for), блочные структуры и подпрограммы.
Почти все парадигмы программирования включают структурное программирование, в том числе и ООП.
