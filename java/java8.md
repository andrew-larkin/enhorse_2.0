...
Анонимный класс используется для интерфейсов и абстрактных классов, тогда как лямбда-выражения используются только 
для функциональных интерфейсов.

+ Анонимный класс — это выражение, присвоенное ссылочной переменной класса или интерфейса, который он реализует.
Лямбда-выражение — это метод без имени. Сигнатуру нереализованного метода мы предоставляем в функциональном интерфейсе 
без имени. Нам также не нужно упоминать типы данных аргументов метода. Метод разрешается во время выполнения.
+ В анонимном классе ключевое слово this относится к самому анонимному классу. Но в случае лямбда-выражения this
относится к включающему его классу.
+ Когда мы компилируем лямбда-выражение, к байт-коду добавляется эквивалентный частный статический или 
нестатический метод. Сигнатура этого метода соответствует методу функционального интерфейса. При компиляции для 
каждого анонимного класса создается отдельный файл класса.
+ Лямбда-выражение лучше по сравнению с анонимным классом с точки зрения производительности. 
Это связано с тем, что анонимный класс приводит к созданию дополнительного файла класса при компиляции, что 
требует дополнительного времени при загрузке класса и проверке во время выполнения.
